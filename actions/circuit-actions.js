...
export function getCircuitSeasonQualifyingResults(circuit, season = "current") {
  return requestData(`/api/circuit/qualifying-results?circuitId=${circuit}&year=${season}`, setCircuitQualifyingResults);
}

export function getCircuitInformation(circuit) {
  return requestData(`/api/circuit/circuit-information?circuitId=${circuit}`, setCircuitInformation)
}
